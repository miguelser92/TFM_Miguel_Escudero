...
DEFAULT_OUT     = r'C:\Users\Miguel\OneDrive\MASTER\11_TFM\Código\reports\good_report20.pdf'
...
def make_file_page(
    ...
    fig.suptitle(
        f"Good file {file_id}   |   {n_events:,} events   |   Dead sensor: {dead_str}",
...
def make_accumulated_page(
    ...
    # ── Título de la página ───────────────────────────────────
    fig.suptitle(
        f"Godd file {file_id}   |   flood-map signal regrouped per sensor area (v2.0)",
        fontsize=14, fontweight='bold',
    )
...
def main():
    parser = argparse.ArgumentParser(description='Informe PDF de diagnóstico de archivos Good')
    ...
    with PdfPages(out_path) as pdf:
        ...
        # Metadatos del PDF
        d = pdf.infodict()
        d['Title']  = ' files diagnostic report'
