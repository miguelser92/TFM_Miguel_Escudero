...
Para cada archivo datas#.dat detecta el/los SiPM averiados y produce una página con:
...
Además, una página resumen al inicio con la tabla archivo → sensor(es) muerto(s).
...
DEFAULT_BAD_DIR = r'E:\Datos TFM\Good\Good'
DEFAULT_PSIPM   = r'E:\Datos TFM\psipm.tsv'
DEFAULT_OUT     = r'C:\Users\Miguel\OneDrive\MASTER\11_TFM\Código\reports\good_report.pdf'
...
def make_file_page(
    ...
    fig.suptitle(
        f"Godd file {file_id}   |   {n_events:,} events   |   Dead sensor: {dead_str}",
...
def make_summary_pages(pdf: PdfPages, summary: list):
    ...
    for start in range(0, n, rows_per_page):
        ...
        ax.axis('off')
        ax.set_title('Good files — dead sensor summary', fontsize=15, fontweight='bold', pad=20)
...
def main():
    ...
    with PdfPages(out_path) as pdf:
        ...
        # Metadatos del PDF
        d = pdf.infodict()
        d['Title']  = 'Good files diagnostic report'
