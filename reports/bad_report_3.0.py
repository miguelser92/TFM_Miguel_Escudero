...
bad_report.py  (v3.0)
=====================
v3.0: flood map (Panel 3) con estilo fino (imshow + inferno + PowerNorm, 220 bins),
igual que bad_recovery, para identificar los canales malos a ojo con nitidez. El
resto del informe es idéntico al v2.0.
...
DEFAULT_OUT     = r'C:\Users\Miguel\OneDrive\MASTER\11_TFM\Código\reports\bad_report30.pdf'
...
def make_file_page(
    ...
    # ── Panel 3: flood map XY (v3.0: estilo fino tipo bad_recovery) ──
    # imshow con muchos bins + colormap 'inferno' + PowerNorm(gamma=0.5) para realzar
    # el patrón y que los agujeros/canales muertos se identifiquen a ojo con nitidez.
    ax = axes[1, 0]
    xr = [float(pos_x.min()), float(pos_x.max())]
    yr = [float(pos_y.min()), float(pos_y.max())]
    H, _, _ = np.histogram2d(pos_x, pos_y, bins=220, range=[xr, yr])
    im = ax.imshow(H.T, origin='lower', extent=[xr[0], xr[1], yr[0], yr[1]],
                   cmap='inferno', norm=PowerNorm(gamma=0.5), aspect='equal',
                   interpolation='nearest')
    plt.colorbar(im, ax=ax, label='Counts', fraction=0.046, pad=0.04)
    # Superponemos las posiciones de los SiPMs como referencia para localizar el canal
