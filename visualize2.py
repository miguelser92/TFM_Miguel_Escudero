...
def panel_mean_activity(ax: plt.Axes, X: np.ndarray):
    ...
    for dense_idx in range(N_ACTIVE):
        ...
        patch = mpatches.RegularPolygon(
            ...
            orientation=0,
            facecolor=color, edgecolor=DARK_BG, linewidth=0.5,
        )
        ax.add_patch(patch)
        ax.text(cx, cy + 0.05, f"{ich}", ha='center', va='center',
...
def main():
    ...
    if args.psipm:
        ...
        if psipm_path.exists():
            ...
            for sep in ['\t', ',', ' ', ';']:
                try:
                    _df = pd.read_csv(psipm_path, sep=sep)
            ...
            if df_raw is None:
                print("\u26a0\ufe0f  No se pudo leer psipm.tsv, usando geometría axial por defecto")
            else:
                df_raw.columns = [c.strip().lower() for c in df_raw.columns]
                print(f"   \u2192 Columnas detectadas en psipm: {list(df_raw.columns)}")

                # Detectar columnas por nombre, con fallback por posición
                cols = list(df_raw.columns)
                ich_col = xsipm_col = ysipm_col = None
                for col in cols:
                    cl = col.replace(' ', '').replace('_', '')
                    if cl in ('ich', 'channel', 'ch', 'id', 'sipm', 'nsipm', 'n', 'index'):
                        ich_col = col
                    elif cl.startswith('x') or cl in ('posx', 'px'):
                        xsipm_col = col
                    elif cl.startswith('y') or cl in ('posy', 'py'):
                        ysipm_col = col

                if ich_col   is None: ich_col   = cols[0]
                if xsipm_col is None: xsipm_col = cols[1]
                if ysipm_col is None: ysipm_col = cols[2]

                print(f"   \u2192 Mapeo: ich='{ich_col}'  x='{xsipm_col}'  y='{ysipm_col}'")

                df_raw = df_raw.rename(columns={ich_col: 'ich', xsipm_col: 'xsipm', ysipm_col: 'ysipm'})
                df_raw['ich'] = pd.to_numeric(df_raw['ich'], errors='coerce')
                df_raw = df_raw.dropna(subset=['ich'])
                df_raw['ich'] = df_raw['ich'].astype(int)
                psipm = df_raw[df_raw['ich'].isin(ACTIVE_CH)].set_index('ich')
                print(f"   \u2192 psipm.tsv cargado: {len(psipm)} SiPMs")
        else:
            print("\u26a0\ufe0f  psipm.tsv no encontrado, usando geometría axial por defecto")
