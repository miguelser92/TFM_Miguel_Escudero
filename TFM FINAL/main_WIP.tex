...
the intermediate geometry therefore buys a small advantage on its own terrain
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.9\textwidth]{multidead_curve}
\caption{Recovery and residual error against the number of simultaneously
suppressed channels, for the model trained on \emph{near} with one to four
channels and evaluated under the three failure geometries. Left, recovery at
P90; right, absolute displacement at P90, with the degraded case dashed for
reference. The two panels show the opposing readings discussed in the text:
contiguous failures give the highest recovery and, at the same time, the largest
residual error.}
\label{fig:multidead}
\end{figure*}
...
failures across the array keeps each individual deficit small. The intermediate
...
% ---------------------------------------------------------------------
% BOCETO v1. Seis subsecciones. Ninguna cifra que no este ya en Resultados.
...
measurements point away from the model. Nine architectural variants, a doubled
training budget and a larger parameter count all leave recovery within a point
of the reference. What does move it, and by far, is which detectors the model
is measured on: recovery varies between 29 and 63\,\% across individual
modules, with a standard deviation of 7.9 points, against 0.3 between training
runs of the same partition.

That comparison sets the terms in which any figure here should be read. The
relative ordering of models is measured precisely, because every comparison is
paired over the same modules, events and masks. The absolute value on a new
detector is far less certain, and the dominant term is hardware variability
rather than anything about the network. It also suggests where the remaining
margin lies: not in a better model, but in accounting for the differences
between modules, which is what a per-module calibration would do.

\subsection{Behaviour Under Multiple Failures}

Training on single failures does not transfer to multiple ones, and the gap is
the largest effect we measured, 24.6 points. That much was expected. Two
observations were not.

The first is that the geometry used to generate the training masks barely
matters once the model sees multiple failures at all. The intermediate geometry
gains 1.99 points on its own terrain and is indistinguishable from the
contiguous one elsewhere. The practical recommendation that follows is
therefore simpler than we anticipated: train with one to four channels
suppressed, and do not worry much about the pattern.

The second is that the energy spectrum does not degrade as failures accumulate.
Spectral recovery rises from 83 to 89\,\% between one and four suppressed
channels, because the damage grows faster than the residual. The metric is a
ratio, and a larger fault offers more to repair. Read in absolute terms the
picture is the ordinary one --- four contiguous failures leave 0.63 ADC units of
residual shift against 0.20 for four scattered ones --- but the operational
consequence is worth stating: the energy information degrades gracefully, which
is what a downstream energy window needs.

The two metrics also disagree on which geometry is hardest, and the
disagreement is instructive. Contiguous groups are the easiest to correct in
position and the hardest in energy. A compact group displaces the centroid
enormously but predictably, since the surviving ring around it still describes
where the light went; the same group removes a large amount of charge from one
region, and that is harder to restore. Scattered failures behave the opposite
way. This is a reminder that a single figure of merit would have hidden half of
...
light sharing induces among its neighbours. That the reconstruction is possible
at all is a consequence of the monolithic geometry.

% TODO(discusion): decidir si comparar con Jimenez Algarra y Hidalgo-Torres.
% Trabajan sobre este mismo detector pero resuelven POSICIONAMIENTO, no
% imputacion, y con datos simulados. La comparacion util no es de cifras
% -no son comparables- sino de encaje: su red estima la posicion a partir de
% los 61 canales, la nuestra repara el vector de 61 canales antes de que esa
% estimacion ocurra. Son piezas consecutivas del mismo sistema.

\subsection{Limitations}

Four limitations bound what the preceding results support.
...
approximately 3.5 points, and it is the reason we report cross-validated values
rather than the result of a single partition.

No energy calibration was applied. The spectral metric is constructed as a
paired difference so that the missing calibration cancels, but this means the
results describe the conservation of the spectrum rather than its absolute
accuracy in keV.

The simulated fault is a total suppression. Real degradation is often partial:
a channel that retains part of its response, or drifts with temperature. The
...
Finally, the worst-channel figure turned out to be unusable as a robustness
metric. Its variation across training runs, $\pm 4.5$ points, exceeds any
difference we could have attributed to a design choice, and the channel
attaining the minimum is not stable. We report it descriptively and draw no
conclusion from it.

\subsection{Future Work}

Three directions follow directly. A per-module calibration would address the
dominant source of uncertainty identified above. Extending the training masks
to partial degradation would cover the failure mode the current model does not
see. And the 62 modules with genuine defects, of which 18 are annotated so far,
offer a validation against expert criterion that no synthetic injection can
replace.
