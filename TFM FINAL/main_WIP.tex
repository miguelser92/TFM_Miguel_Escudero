...
% --- Numeracion de secciones: 1, 1.1, 1.1.1 en vez de I, A, 1) -------
...
silicon photomultipliers is optically coupled to its rear face
\cite{perez-benito_sipm-based_2019}. The array provides 64 readout channels, of
which 61 are instrumented; the remaining three are inactive and we excluded
them throughout. The active sensors tile a hexagonal lattice with a pitch of
3.75\,mm and span 26\,mm across, so that they do not reach the crystal
boundary. The lateral faces of the crystal carry an absorbing black adhesive
...
fraction of their light, which shifts both the charge recorded by the outermost
...
\end{equation}
%
where $\mathcal{N}(c)$ are the neighbours of sensor $c$ and $\sigma$ is a GELU
nonlinearity \cite{hendrycks2016gelu}. The aggregation is isotropic: every
...
We also evaluated four alternative graph formulations, to test whether the
isotropic mean is what limits performance: attention over neighbours
\cite{velickovic2018gat}, multiple aggregation statistics combined with degree
scalers \cite{corso2020pna}, an encoder--decoder with graph pooling
\cite{gao2019graphunet}, and a graph transformer with global attention
...
We trained with the mean squared error computed over the full 61-channel
...
mean absolute error and the Huber loss \cite{huber1964} with $\delta = 0.1$.
...
At the charge level we report the mean absolute error on the imputed channel in
normalised units, together with the bias, which acts as a guardrail: a model
...
image. For each event we computed the interaction position by centre of gravity
weighted by the squared charge \cite{anger1958, poladyan2020}. We then measured
the displacement $\Delta R$ between the position given by the intact event and
the position given by the faulty one, both before and after imputation. The recovery is the fraction of that displacement the model
removes,
%
\begin{equation}
...
events. Note that both displacements are measured against the intact event, so
the numerator is already the residual error of the imputation; the denominator
normalises it by the damage the fault caused. We report the value at the 90th
percentile, which weights the difficult events, and also the mean. Averaging
over the 61 channels gives the macro figures used throughout, and the minimum
over channels characterises the worst case. Because a relative measure hides
the magnitude of what remains, we report $\Delta R_{\text{imp}}$ in millimetres
as well.

Two further analyses address physical fidelity rather than accuracy. The first
measures the point-spread function of the flood map, comparing its full width
at half maximum before and after imputation, which connects the method to the
spatial resolution of the detector \cite{moses2011}. The second measures
whether the energy spectrum survives imputation, evaluated by zones of the
crystal defined through concentric hexagonal bands, since the black adhesive on
the lateral faces makes the periphery behave differently from the centre
\cite{conde_minimization_2014}. This second analysis uses a paired differential
design: over the same events we subtract the true channel and add back the
prediction, so that the absence of energy calibration affects both terms
...
Fig.~\ref{fig:floodmap-res}, which compares the flood map of a test module with one
...
Adding a term that penalises the displacement of the reconstructed position
leaves recovery unchanged at 58.23\,\% against 58.02\,\% for the reference. Adding a term on
...
% NOTA (24/08): la subseccion "Application to Faulty Modules" se ha retirado
% junto con el estadistico z-score de Metodos. No se menciona en la
% introduccion y el trabajo no lo usa, asi que quedaba huerfano. El material
% (ROC, TPR de 1.00 a severidad total, etiquetado de 18/62 modulos) sigue en
% reports/bad_validation.json por si se decide reincorporarlo.
