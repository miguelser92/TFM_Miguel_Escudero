...
\begin{abstract}
The spatial resolution of Positron Emission Tomography (PET) limits its use in
the small structures that dedicated and organ-specific systems target, and
monolithic scintillator crystals have gained ground there because they preserve
the continuous distribution of the scintillation light instead of segmenting
it, which allows the interaction position to be estimated between sensors
rather than at a fixed pitch. That advantage carries a dependency: the light of
...
There is no established figure for this task to compare against, which is not
surprising: monolithic detectors remain uncommon outside research settings, and
where a dead channel is handled at all it is masked rather than recovered. We
offer ours as a starting point. Three lines follow from it --- a calibrated
