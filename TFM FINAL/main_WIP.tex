...
coupled to an array of Hamamatsu silicon photomultipliers
\cite{perez-benito_sipm-based_2019}, each channel comprising 3980 microcells
with a photon detection efficiency of approximately 35\,\% at the emission
wavelength of LYSO \cite{jimenez2025}. The readout electronics convert the
detected light into a digitised charge $R_{ch}$ per channel
\cite{gundacker2020}, which is the quantity this work operates on. The lateral
...
Both the photosensor array and the readout electronics are prototypes developed
within the group and still under active development, so we describe them
functionally rather than by commercial reference.
...
one binary file per physical detector module. The collection comprises 159 nominally healthy
...
% NOTA: la duracion de cada adquisicion no consta y se omite deliberadamente
% (decision de Miguel, 21/08): mejor omitirla que tomarla de otro montaje.
% Verificado en los ficheros que la irradiacion es en INUNDACION y no el barrido
% colimado de Jimenez Algarra: al partir un fichero en 61 bloques temporales el
% centroide medio no se mueve (sd 0,05 mm) y cada bloque cubre todo el cristal.
%
% El modelo de SiPM y de ASIC quedan descritos funcionalmente y no por
% referencia comercial: son prototipos del grupo en desarrollo (decision de
% Miguel, 21/08), igual que en Jimenez Algarra.
