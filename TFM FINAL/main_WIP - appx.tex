...
inspected the flood maps of the 62 defective modules and annotated 18 of them
channel by channel (Appendix~\ref{app:bad}), we found three recurring
...
collected light, fall below 45\,\%. Appendix~\ref{app:sensor} separates that
gradient from the statistics available for each sensor, which vary by a factor
of four across the array.
...
evaluation; their configurations are given in Appendix~\ref{app:arch}. The two models without a geometric prior reach 51.39\,\% and
...
38\,k reference. PNA, which combines four aggregation statistics over each
neighbourhood, matches the reference at 58.45\,\% rather than exceeding it, using
...
larger. The five partitions are listed in Appendix~\ref{app:splits}. Evaluating the model on individual unseen modules explains why:
...
recovery of $84.7 \pm 0.9$\,\% ($n=3$; the histograms behind this figure are
in Appendix~\ref{app:spectra}). Here the
...
%
% MOVIDA AL APENDICE D (app:spectra). Aqui ocupaba una pagina entera en mitad
% del argumento y con quince paneles no se leia. En el apendice va junto a su
% version con k=4, que es donde la comparacion dice algo.
% Para devolverla a Resultados: descomentar este bloque y borrar el gemelo del
% apendice, que la etiqueta fig:espectro no puede estar definida dos veces.
%
% \begin{figure*}[t]
% \centering
...
the patterns we observed in the flood maps of the 62 defective modules of the
batch, extends that behaviour to several dead channels at no cost in the
single-channel case.

What this work leaves is a working baseline, not a closed problem. The ceiling
we measured is set by the data rather than by the network: real acquisitions,
uncalibrated in energy, and a fault that has to be injected because a genuine
one leaves no reference against which to measure the error. Each of those
points to a line of work. A calibrated or controlled acquisition would separate
what belongs to the method from what belongs to the hardware; the annotated
defective modules offer a validation against expert criterion that no synthetic
fault can provide; and propagating imputed events through a full reconstruction
is the step that would turn a per-detector correction into a system-level
result. What can be stated already is that the information needed to repair a
dead channel is present in its neighbours, and that a small network with the
geometry of the detector built into it is enough to extract it.


% ---------------------------------------------------------------------
% APENDICES
% ---------------------------------------------------------------------
% \appendices numera A, B, C... y va SIEMPRE antes de la bibliografia.
% Criterio de que entra aqui: evidencia primaria y reproducibilidad, y solo
% si el cuerpo del texto lo cita. Lo que no se cita, no va.
\appendices

\section{Architecture Configurations}
\label{app:arch}

Table~\ref{tab:architectures} compares eight models by parameter count and
recovery, but those two numbers do not determine an architecture. Table
\ref{tab:app-arch} gives the configuration behind each row. All eight share the
optimiser, learning rate, schedule, batch size and sample budget of
Table~\ref{tab:hyperparams}, and all receive the same input: two features per
node, the normalised charge and the binary mask that marks the suppressed
channels. They differ only in how they propagate information between sensors.

\begin{table}[t]
\centering
\caption{Configuration of the models compared in
Table~\ref{tab:architectures}. Width is the hidden dimension; all models use
GELU activations and dropout 0.1.}
\label{tab:app-arch}
\begin{tabular}{@{}llr@{}}
\toprule
\textbf{Model} & \textbf{Configuration} & \textbf{Params} \\
\midrule
DeepMLP           & Widths 512--512--256--128, batch norm & \num{500541} \\
ResidualMLP       & Width 256, 4 residual blocks          & \num{345981} \\
Graphormer        & Width 256, 8 layers, 8 heads          & \num{4263881} \\
Graph U-Net       & Width 128, global bottleneck          & \num{331649} \\
PNA               & Width 64, 4 blocks, 4 aggregators     & \num{165761} \\
\textsc{HexGNN-s} & Width 48, 4 blocks                    & \num{38305} \\
\textsc{HexGNN-m} & Width 96, 6 blocks                    & \num{225217} \\
\textsc{HexGNN-l} & Width 128, 6 blocks                   & \num{398593} \\
\bottomrule
\end{tabular}
\end{table}

Two entries need a comment. The Graphormer attends over all 61 nodes at every
layer, and the inter-sensor distance matrix enters the attention as a bias, so
the geometry reaches it as a soft prior rather than as a fixed adjacency. The
PNA variant reported here aggregates the mean, maximum, minimum and standard
deviation of each neighbourhood, without the degree scalers of the original
formulation. We also trained the scaled version: it needs 2.6 times more
parameters and recovers 56.91\,\% against the 57.13\,\% of the unscaled one on
the common evaluation, so the scalers do not pay for themselves on a graph
whose degree varies only between three and six.

\section{Data Partitions}
\label{app:splits}

The five partitions of Table~\ref{tab:reference} are not stored as lists of
files. They are generated by a single deterministic function, shared by the
training script and every evaluation, which shuffles the 159 healthy modules
with a fixed seed and takes the first five as test and the next five as
validation. Reporting the seed therefore defines the partition exactly. Table
\ref{tab:app-splits} lists the modules each seed produces, so that a specific
result can be traced to specific detectors.

\begin{table}[t]
\centering
\caption{Held-out modules of the five partitions. The remaining 149 modules
form the training set in each case.}
\label{tab:app-splits}
\small
\begin{tabular}{@{}clll@{}}
\toprule
\textbf{Part.} & \textbf{Seed} & \textbf{Validation} & \textbf{Test} \\
\midrule
A & 42 & 013, 015, 080, & 057, 116, 126, \\
  &    & 142, 182       & 202, 214 \\
B & 43 & 083, 105, 127, & 037, 045, 101, \\
  &    & 152, 188       & 122, 172 \\
C & 44 & 067, 147, 155, & 043, 138, 140, \\
  &    & 180, 199       & 142, 150 \\
D & 45 & 067, 150, 177, & 012, 095, 136, \\
  &    & 202, 211       & 142, 160 \\
E & 46 & 081, 090, 126, & 077, 085, 129, \\
  &    & 167, 214       & 141, 215 \\
\bottomrule
\end{tabular}
\end{table}

The partitions are drawn independently, so a module can appear in the held-out
set of more than one of them: \texttt{datas142} is a test module in C and D and
a validation module in A. This is deliberate, since the quantity being
estimated is the spread over partitions and not a partition of the batch into
disjoint folds, but it means the five values of Table~\ref{tab:reference} are
not fully independent and the $\pm 2.32$ band should be read as a lower bound.

One further caveat belongs here. Partition A was trained under the protocol
described in Section~\ref{sec:training}, one module per epoch over 149 epochs
of \num{107000} events. Partitions B to E predate it and were trained with 40
epochs of \num{400000} events drawn from 40 modules. The sample budget is
almost identical, \num{15943000} against \num{16000000}, a difference of
0.36\,\%, but the module coverage is not: B to E saw 40 of the 149 training
detectors and A saw all of them. Repeating the cross-validation under the
current protocol is pending. We do not expect it to narrow the band, which is
dominated by having five modules in the test set rather than by the training
schedule, but it would make the five figures homogeneous.

\section{Annotated Defective Modules}
\label{app:bad}

The batch contains 62 modules that the acquisition software flagged as
defective. We inspected their flood maps and annotated 18 of them channel by
channel, which is the evidence behind the failure regimes of
Section~\ref{sec:regimes}. Table~\ref{tab:app-bad} lists the annotation. Of the
18, fourteen have at least one channel we could identify as dead and four show
a distorted flood map with no individual channel we were willing to name, which
is itself a result: a defective flood map does not always reduce to a dead
sensor.

\begin{table}[t]
\centering
\caption{Manually annotated defective modules. Channels are given in the
detector's own \textit{Ich} numbering.}
\label{tab:app-bad}
\small
\begin{tabular}{@{}lcl@{}}
\toprule
\textbf{Module} & \textbf{Dead} & \textbf{Channels (Ich)} \\
\midrule
\texttt{datas008} & 1 & 7 \\
\texttt{datas010} & 1 & 19 \\
\texttt{datas024} & 1 & 27 \\
...
\texttt{datas041} & 1 & 62 \\
\texttt{datas050} & 1 & 34 \\
\texttt{datas001} & 2 & 14, 49 \\
\texttt{datas028} & 2 & 13, 52 \\
\texttt{datas032} & 2 & 19, 61 \\
\texttt{datas042} & 2 & 0, 25 \\
\texttt{datas046} & 2 & 7, 30 \\
\texttt{datas029} & 5 & 0, 32, 44, 61, 62 \\
\texttt{datas016} & 7 & 36, 41, 42, 43, 51, 59, 61 \\
\midrule
\multicolumn{3}{@{}l}{\texttt{datas009}, \texttt{datas017}, \texttt{datas025},
\texttt{datas051}: no channel identified} \\
\bottomrule
\end{tabular}
\end{table}

This distribution is what sets the range of $k$ used throughout the paper.
Twelve of the fourteen annotated modules have one or two dead channels, and
none has three or four; training over $k=1\dots4$ therefore covers every case
we observed with margin, while the two modules with five and seven dead
channels fall outside it. Whether the method still holds there is open, and
\texttt{datas016}, with seven dead channels out of 61, is the case we would
test first. The 29 dead channels are spread over 24 distinct sensors, with only
Ich 0, 7, 19, 61 and 62 failing in more than one module, so the annotation does
not point to a systematically fragile position in the array.

\section{Complete Energy Spectra}
\label{app:spectra}

Section~\ref{sec:res-fidelity} reports spectral recovery as a single figure per
condition. Figs.~\ref{fig:espectro} and \ref{fig:espectro-k4} give the
histograms those figures summarise, for one and for four suppressed channels.
The comparison between the two is the visual form of the result in
Table~\ref{tab:espectro-k}: quadrupling the damage does not visibly separate
the imputed curve from the original one.

\begin{figure*}[p]
\centering
\includegraphics[width=\textwidth]{espectros_zonas}
\caption{Energy spectra of the affected events for five channels (columns) and
the three crystal zones (rows), with \textbf{one} suppressed channel, comparing
the original distribution in grey with the degraded one in orange and the
imputed one in blue. The 511\,keV photopeak of $^{22}$Na is visible in the core
and middle rows and flattens towards the edge, where the absorbing adhesive
removes part of the collected light. The degraded curve departs from the
original towards lower energies, and imputation brings it back; the per-panel
figure is the spectral recovery.}
\label{fig:espectro}
\end{figure*}

\begin{figure*}[p]
\centering
\includegraphics[width=\textwidth]{espectros_zonas_k4}
\caption{The same measurement with \textbf{four} simultaneously suppressed
channels in the \emph{near} geometry, on the same axes as
Fig.~\ref{fig:espectro}. The orange curve is displaced further, as expected,
but the blue one still tracks the grey: recovery rises from
$83.1 \pm 0.2$\,\% to $88.7 \pm 0.3$\,\% because the damage grows faster than
what remains of it after imputation.}
\label{fig:espectro-k4}
\end{figure*}

\section{Per-Sensor Statistics}
\label{app:sensor}

Recovery is reported per channel in Fig.~\ref{fig:mapa} and degrades from the
core towards the edge. Fig.~\ref{fig:app-stats} separates two quantities that
the recovery map combines. The left panel gives the number of modified events
available for each sensor, which ranges from \num{459000} at a corner to
\num{1892000} at the centre; the right panel gives the imputation error in ADC
units rather than as a fraction of the damage.

Read together they answer a question the recovery map leaves open. The edge
sensors are not worse because they were measured less often: the ratio in
statistics is fourfold and the ratio in MAE is under twofold, and the pattern
of the right panel follows the geometry of the array rather than that of the
left. What the edge lacks is neighbours, not events.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{stats_por_sensor}
\caption{Left, number of modified events per sensor over the test partition.
Right, mean absolute imputation error per sensor in ADC units. Both over the
61 active channels of the reference model.}
\label{fig:app-stats}
\end{figure*}
