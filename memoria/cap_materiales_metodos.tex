...
para test queda apreciablemente fuera de ese régimen. Esta observación es relevante al interpretar
tanto el comportamiento del canal $\mathrm{Ich}\,31$ (Sección~\ref{sec:discusion}) como el techo de
rendimiento descrito en los resultados: parte de la limitación podría proceder de la cobertura del
entrenamiento y no exclusivamente de la información físicamente disponible. Ampliar el presupuesto
de épocas, o barajar el orden de rotación, permitiría discriminar entre ambas explicaciones y
constituye la comprobación pendiente más inmediata.
...
\quad DeepMLP & \num{500541} & --- & 50.51 & 0.737 & --- \\
\quad ResidualMLP & \num{345981} & --- & 48.30 & 0.741 & --- \\
\midrule
\multicolumn{6}{l}{\emph{Red de grafos geométrica}} \\
\quad HexGNN \texttt{s} (referencia) & \num{38305} & 52.66 & 58.02 & 0.6195 & 29.1 \\
\quad HexGNN \texttt{m} & \num{225217} & --- & 57.92 & 0.627 & --- \\
\quad HexGNN \texttt{l} & \num{398593} & --- & 57.15 & 0.650 & --- \\
\midrule
\multicolumn{6}{l}{\emph{Variantes arquitectónicas}} \\
\quad PNA & \num{165761} & 52.48 & 58.45 & 0.6309 & 29.6 \\
\quad Convolución anisótropa & \num{130465} & 52.26 & 57.26 & 0.6293 & 29.5 \\
\quad U-Net de grafo & \num{331649} & 47.95 & 55.22 & 0.6918 & 32.7 \\
\quad Graphormer & \num{4234113} & 49.65 & 55.17 & 0.6600 & 30.9 \\
\quad Normalización por canal & \num{38305} & 50.40 & 55.87 & 0.6442 & 30.3 \\
\midrule
\multicolumn{6}{l}{\emph{Ensembles}} \\
\quad Ensemble homogéneo ($3\times$) & $3\times$\num{38305} & 52.93 & 58.69 & \textbf{0.6110} & \textbf{28.7} \\
\quad Ensemble heterogéneo ($3\times$) & --- & \textbf{53.21} & \textbf{58.73} & 0.6146 & 28.9 \\
\bottomrule
\end{tabular}
\end{table}

% TODO: Completar las celdas marcadas con "---" correspondientes a la recuperación en media y
% al error relativo de DeepMLP, ResidualMLP y HexGNN m/l. Estos valores requieren reejecutar
% eval_total.py sobre dichos modelos con la campaña homogénea de 600 000 eventos, dado que sus
% evaluaciones disponibles se realizaron con 200 000 eventos por fichero.
...
$\mathcal{R}_{\text{media}} = 10.57$, MAE $= 1.31$ y error relativo del 61,5\,\%. La recuperación
...
\paragraph{Estadística de módulos reducida.} La partición reserva únicamente cinco módulos para
test. Aunque la estadística de eventos es holgada, las conclusiones referidas a canales individuales
pueden reflejar particularidades de esos módulos concretos. El caso del canal $\mathrm{Ich}\,31$
ilustra el problema: identificado inicialmente como canal sistemáticamente anómalo, el análisis
posterior reveló que su comportamiento en los módulos de entrenamiento es normal y que la anomalía
se concentra en dos módulos que resultaron asignados a la partición de test. La interpretación
correcta no es la de un canal defectuoso, sino la de un ejemplo de la variabilidad de ganancia entre
módulos, y por tanto de la necesidad de calibración.

\paragraph{Ausencia de validación cruzada.} No se ha realizado validación cruzada por coste
computacional, lo que impide estimar la sensibilidad de los resultados a la partición concreta.
...
población completa. Todas las comparaciones entre modelos son válidas, puesto que comparten
protocolo, pero el rendimiento \emph{absoluto} está previsiblemente subestimado y la atribución del
techo observado a un límite de información física queda pendiente de comprobación con cobertura
completa.
...
% TODO(4): Completar las celdas de la Tabla \ref{tab:res-arquitecturas} marcadas
%          con "---", reejecutando eval_total.py sobre DeepMLP, ResidualMLP y
%          HexGNN m/l con la campaña homogénea de 600 000 eventos por fichero.
