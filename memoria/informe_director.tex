...
\textbf{52.6\,\%} del desplazamiento de posición, y el \textbf{58.1\,\%} en el 10\,\% de
eventos peores. El modelo tiene \textbf{38\,305 parámetros}, un tamaño muy modesto.
\end{idea}
...
Para poner esa cifra en contexto: entrenando tres veces el mismo modelo con distinta semilla
de inicialización, la dispersión de los resultados es de apenas \textbf{0.09 puntos}. La
variabilidad debida a \textbf{qué detector} se mide es, por tanto, unas \textbf{ochenta veces
mayor} que la debida a cómo se entrenó.

\begin{idea}
\noindent\textbf{Qué cambia y qué no.} Las comparaciones entre modelos siguen siendo
válidas, porque todas se hacen sobre \emph{los mismos} cinco módulos: el efecto del detector
se cancela al comparar. Lo que no puede afirmarse es que el método recupere el 58\,\% en un
detector cualquiera; esa extrapolación tiene una incertidumbre de $\pm 3.5$ puntos y una
dispersión de $\pm 7.9$ entre unidades. Es la diferencia entre \emph{precisión relativa},
que es excelente, y \emph{exactitud absoluta}, que es mucho más incierta.
\end{idea}
